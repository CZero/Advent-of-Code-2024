...
func (m *StringsMatrix) calcAntinodes() {
	for _, coords := range m.antennas {
		...
		for _, combi := range combis {
			...
			if m.inmatrix(Coord{combi[1].r - rVector, combi[1].c - cVector}) {
				m.antinodes[Coord{combi[1].r - rVector, combi[1].c - cVector}] = true
			}
			if m.inmatrix(Coord{combi[0].r + rVector, combi[0].c + cVector}) {
				m.antinodes[Coord{combi[0].r + rVector, combi[0].c + cVector}] = true
			}
		}
	}
}
